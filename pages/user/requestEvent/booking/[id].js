...
export default function PaymentDetails() {
    ...
    const wipayPayment = () => {
        ...
        if(totalPrice > 0){
            ...
            .catch(error => console.log("outside"));
        }
        // if ticketprice is 0
        else{
            var myHeaders = new Headers();
    ...
    useEffect(()=>{
        ...
        if(eventId){
            fetch(process.env.URL+"/user/request/"+eventId, requestOptions)
            .then(response => response.json())
            .then(result => {    
                setData(result.requests);
                let obj = {
                    "startDateTime": result.requests.slot.startDateTime,
                    "endDateTime": result.requests.slot.endDateTime
                };
                setSlot(obj);
                if(result.requests.tickets){
                    let selectedTickets = [];
                    let tickCount = 0;
                    let price = 0;
                    result.requests.tickets.map((item)=>{
                        let obj2 = {
                            "type": item.type,
                            "price": item.price,
                            "count": item.count
                        ...
                        selectedTickets.push(obj2);
                    })
                    setTickets(selectedTickets)
                    feesCalculator(price)
                    setTicketCount(tickCount);
                    setTotalPrice(price);
                }
            })
            .catch(error => console.log('error', error));
            ...
            <>
                {!cardPage && <> 
                    <div className={`container bg-white rounded-20 p-5`}>
                        ...
                        <form onSubmit={detailHandler} className={`${styless["payment-form"]} d-flex  d-flex-wrap d-justify-space-between`}>
                            ...
                            <div className={`col-12 d-flex d-flex-wrap pl-2 pr-2 ${styless["slots-wrapper"]}`}>
                                <li className={`col-12 user-select-none d-flex-wrap `}>
                                    <div className={`cursor-pointer ${styless["slots"]} ${styless["active"]}`} >
                                        <span className="text-secondary font-14 f-300">
                                            <Moment utc format="dddd MMMM D">{slot.startDateTime}</Moment>
                                        </span>
                                        <span className="text-primary font-12 f-600 mt-1">
                                            <Moment utc format="hh:mm A" >{slot.startDateTime}</Moment> - <Moment utc format="hh:mm A" >{slot.endDateTime}</Moment> IST
                                        </span>
                                    </div>
                                </li>
                            </div>
                            ...
                            <div ref={ticketsRef} className="col-12 col-md-6 mt-3 pr-5 pr-2 pl-2 d-flex d-flex-wrap">
                               ...
                               {tickets.map((item,index)=>{ return <div key={index} className="col-12 mt-1 d-flex d-align-center ">
                                    <div className="col-9 pr-3">
                                        <input type="text" readOnly className="col-12" placeholder={`${item.type} ${currency == "USD"?" $":" TT$"} ${item.price.toFixed(2)}`}/>
                                    </div>
                                    <div className="col-3 d-flex d-justify-space-between d-align-center">
                                        <div role="button" className='cursor-pointer d-flex d-align-center'>
                                        <MinusWithBg></MinusWithBg>
                                        </div>
                                        <h4 className="f-500 l-24 m-0 user-select-none">{item.count}</h4>
                                        <span role="button" className='cursor-pointer d-flex d-align-center'>
                                        <PlusWithBg></PlusWithBg>
                                        </span>
                                    </div>
                                </div>})}
                                <h4 className="f-700 l-20  mb-0 col-12 mt-3">Rewards</h4>
                            ...
                            <div className={`col-12 col-md-6 mt-4 pl-2`}>
                                ...
                                <div className="d-flex d-justify-space-between d-align-center font-16 l-24 f-500">
                                    <span>Voucher</span>
                                    <span className="text-success">-{currency=="USD"?"$":"TT$"} {discount.toFixed(2)}</span>
                                </div>
                                <div className="d-flex d-justify-space-between d-align-center font-16 l-24 f-500">
                                    <span>Booking fees</span>
                                    <span className="text-light-grey">{currency=="USD"?"$":"TT$"} {parseFloat(bookingFees).toFixed(2)}</span>
                                </div>
                ...
                {verifySuccessModal && <VerifySuccess title="Congratulations" subtitle={`You successfully completed your booking for ${data.event.title} event. Check your mail for booking details`} button="View Ticket" handler={successToBookingPageHandler}/>}
            </>
        )
    }
}
