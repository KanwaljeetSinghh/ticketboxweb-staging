...
export default function DropDown(props){
    ...
    const selectHandler = (e) => {
        ...
        props.handler(e.currentTarget.getAttribute("itemID"),e.currentTarget.getAttribute("value"));
    }
        ...
        <div className={`${styles["drop-down"]}`} onClick={handler} style={{backgroundColor:props.color}}>
            ...
            {props.data && <ul>
                ...
                {props.data.map((item, index) => { 
                    return (
                        <li value={item.name} itemID={item._id} onClick={selectHandler} key={index}>
                            {item.name}
                        </li>
                    )
                })}
