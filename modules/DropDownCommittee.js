...
export default function DropDownCommittee(props){
    ...
    const selectHandler = (e) => {
        ...
        props.handler(e.currentTarget.getAttribute("itemID"));
    }
    return (
        <div className={`${styles["drop-down"]}`} onClick={handler} style={{backgroundColor:props.color}}>
            ...
            {!props.data && <ul>
                ...
                <li value="Bhopal" onClick={selectHandler}>
                    Bhopal
                </li>
                <li value="NEWYORK" onClick={selectHandler}>
                    NEWYORK
                </li>
                <li value="Mumbai" onClick={selectHandler}>
                    Mumbai
                </li>
            </ul>
            }
            ...
            {props.data && <ul>
                {props.data.map((item, index) => { 
                    return (
                        item.user && <li value={item.user.name} itemID={item.user._id} onClick={selectHandler} key={index}>
                            {item.user.name}
                        </li>
                    )
                })}
